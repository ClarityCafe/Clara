    ...
    main: (bot, ctx) => {
        return new Promise((resolve, reject) => {
            if (ctx.msg.mentions.length > 2 || ctx.args.length > 2) {
                ctx.msg.channel.createMessage(localeManager.t('ship-noArgs', 'en-UK')).then(resolve).catch(reject);
            } else {
                ...
                ctx.msg.channel.createMessage(localeManager.t('ship', 'en-UK', {result})).then(() => resolve).catch(reject);
            }
        });
    }
};