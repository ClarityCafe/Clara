            ...
            if (ctx.args.length === 0) {
                ...
                osu.get_user({u: ctx.args[0], m: 2}, res => {
                    ...
                    if (user.user_id == undefined) {
                        ...
                        osuser += `Country: "${iso3166.getCountry(user.country)}"\n`;
                        osuser += `Country Rank: ${user.pp_country_rank.replace(commaRegex, ",")}\n`;
                        osuser += `Beatmaps Played: ${user.playcount.replace(commaRegex, ",")}\n`;
                        osuser += `300s Scored: ${user.count300.replace(commaRegex, ",")}\n`;
                        osuser += `100s Scored: ${user.count100.replace(commaRegex, ",")}\n`;
                        osuser += `50s Scored: ${user.count50.replace(commaRegex, ",")}\n`;
                        osuser += `Ranked Score: ${user.ranked_score.replace(commaRegex, ",")}\n`;
                        osuser += `Total Score: ${user.total_score.replace(commaRegex, ",")}\n`;
                        osuser += `PP: ${user.pp_raw.replace(commaRegex, ",")}\n`
                        osuser += `Level: ${Number(user.level).toFixed(1).toString().replace(commaRegex, ",")}\n`;
                        osuser += `Rank: ${user.pp_rank.replace(commaRegex, ",")}\n`;
                        osuser += `Accuracy: ${Number(user.accuracy).toFixed(2)}\n`;
                        osuser += `SS Count: ${user.count_rank_ss.replace(commaRegex, ",")}\n`;
                        osuser += `S Count: ${user.count_rank_s.replace(commaRegex, ",")}\n`;
                        osuser += `A Count: ${user.count_rank_a.replace(commaRegex, ",")}\n`;
                        osuser += "```";
                        ctx.msg.channel.sendMessage(osuser).then(() => {
                            resolve();
                        }).catch(err => reject([err]));
