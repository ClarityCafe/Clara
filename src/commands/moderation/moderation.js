...
function deleteDelay(msg) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            outside.bot.deleteMessage( msg.id).then(() => resolve()).catch(reject);
        }, 1000);
    });
}
                    ...
                    if (ctx.args.length === 0) {
                        ...
                        if (ctx.args[0] === 'all') {
                            if (!ctx.args[1] || !/^\d+$/.test(ctx.args[1])) {
                                ...
                                bot.purgeChannel( Number(ctx.args[1])).then(amt => {
                                    return ctx.msg.channel.createMessage( `Purged **${amt}** ${amt === 1 ? 'message' : 'messages'}.`);
                                }).then(deleteDelay).then(() => resolve()).catch(reject);
                            } else {
                            ...
                            if (!ctx.args[1] || !/^(?:\d+|<@!?\d+>)$/.test(ctx.args[1])) {
                                ...
                                if (!user) {
                                    ...
                                    if (!ctx.args[2] || !/^\d+$/.test(ctx.args[2])) {
                                        bot.purgeChannel( 100, m => m.author.id === user.id).then(amt => {
                                        ...
                                        let i = 0;
                                        bot.purgeChannel( 100, m => m.author.id === user.id && ++i <= Number(ctx.args[2])).then(amt => {
                                            return ctx.msg.channel.createMessage( `Purged **${amt}** ${amt === 1 ? 'message' : 'messages'} from **${bot.formatUser(user)}**.`);
                                        }).then(deleteDelay).then(() => resolve()).catch(reject);
                                    } else {
                                        ctx.msg.channel.createMessage( 'Woah there, way too spicy. I only accept numbers between `1` and `100`').then(() => resolve()).catch(reject);
                            ...
                            if (!ctx.args[1] || !/^\d+$/.test(ctx.args[1])) {
                                bot.purgeChannel( 100, m => m.author.bot).then(amt => {
                                    return ctx.msg.channel.createMessage( `Purged **${amt}** bot ${amt === 1 ? 'message' : 'messages'}.`);
                                }).then(deleteDelay).then(() => resolve()).catch(reject);
                            } else if (/^\d+$/.test(ctx.args[1]) && Number(ctx.args[1]) <= 100 && Number(ctx.args[1]) >= 1) {
                                let i = 0;
                                bot.purgeChannel( 100, m => m.author.bot && ++i <= Number(ctx.args[1])).then(amt => {
                                    return ctx.msg.channel.createMessage( `Purged **${amt}** bot ${amt === 1 ? 'message' : 'messages'}.`);
                                }).then(deleteDelay).then(() => resolve()).catch(reject);
                            } else {
                                ctx.msg.channel.createMessage( 'Woah there, way too spicy. I only accept numbers between `1` and `100`').then(() => resolve()).catch(reject);
                            ...
                            if (!ctx.args[1]) {
                                ...
                                if (!ctx.args[2] || !/^\d+$/.test(ctx.args[2])) {
                                    bot.purgeChannel( 100, m => m.content.includes(ctx.args[1])).then(amt => {
                                        return ctx.msg.channel.createMessage( `Purged **${amt}** ${amt === 1 ? 'message' : 'messages'}.`);
                                    }).then(deleteDelay).then(() => resolve()).catch(reject);
                                } else if (/^\d+$/.test(ctx.args[2]) && Number(ctx.args[2]) <= 100 && Number(ctx.args[2]) >= 1) {
                                    let i = 0;
                                    bot.purgeChannel( 100, m => m.content.includes(ctx.args[1]) && ++i <= Number(ctx.args[2])).then(amt => {
                                        return ctx.msg.channel.createMessage( `Purged **${amt}** ${amt === 1 ? 'message' : 'messages'}.`);
                                    }).then(deleteDelay).then(() => resolve()).catch(reject);
                                } else {
                                    ctx.msg.channel.createMessage( 'Woah there, way too spicy. I only accept numbers between `1` and `100`').then(() => resolve()).catch(reject);
                            ...
                            if (!ctx.args[1] || !/^\d+$/.test(ctx.args[1])) {
                                bot.purgeChannel( 100, m => m.embeds.length > 0).then(amt => {
                                    return ctx.msg.channel.createMessage( `Purged **${amt}** ${amt === 1 ? 'embed' : 'embeds'}.`);
                                }).then(deleteDelay).then(() => resolve()).catch(reject);
                            } else if (/^\d+$/.test(ctx.args[1]) && Number(ctx.args[1]) <= 100 && Number(ctx.args[1]) >= 1) {
                                let i = 0;
                                bot.purgeChannel( 100, m => m.embeds.length > 0 && ++i <= Number(ctx.args[1])).then(amt => {
                                    return ctx.msg.channel.createMessage( `Purged **${amt}** ${amt === 1 ? 'embed' : 'embeds'}.`);
                                }).then(deleteDelay).then(() => resolve()).catch(reject);
                            } else {
                                ctx.msg.channel.createMessage( 'Woah there, way too spicy. I only accept numbers between `1` and `100`').then(() => resolve()).catch(reject);
                            ...
                            if (!ctx.args[1] || !/^\d+$/.test(ctx.args[1])) {
                                bot.purgeChannel( 100, m => m.attachments.length > 0).then(amt => {
                                    return ctx.msg.channel.createMessage( `Purged **${amt}** ${amt === 1 ? 'attachment' : 'attachments'}.`);
                                }).then(deleteDelay).then(() => resolve()).catch(reject);
                            } else if (/^\d+$/.test(ctx.args[1]) && Number(ctx.args[1]) <= 100 && Number(ctx.args[1]) >= 1) {
                                let i = 0;
                                bot.purgeChannel( 100, m => m.attachments.length > 0 && ++i <= Number(ctx.args[1])).then(amt => {
                                    return ctx.msg.channel.createMessage( `Purged **${amt}** ${amt === 1 ? 'attachment' : 'attachments'}.`);
                                }).then(deleteDelay).then(() => resolve()).catch(reject);
                            } else {
                                ctx.msg.channel.createMessage( 'Woah there, way too spicy. I only accept numbers between `1` and `100`').then(() => resolve()).catch(reject);
                            ...
                            if (!ctx.args[1]) {
                                ...
                                if (purgeRegex) {
                                    if (!ctx.args[2] || !/^\d+$/.test(ctx.args[2])) {
                                        bot.purgeChannel( 100, m => purgeRegex.test(m.content)).then(amt => {
                                            return ctx.msg.channel.createMessage( `Purged **${amt}** ${amt === 1 ? 'message' : 'messages'}.`);
                                        }).then(deleteDelay).then(() => resolve()).catch(reject);
                                    } else if (/^\d+$/.test(ctx.args[2]) && Number(ctx.args[2]) <= 100 && Number(ctx.args[2]) >= 1) {
                                        let i = 0;
                                        bot.purgeChannel( 100, m => purgeRegex.test(m.content) && ++i <= Number(ctx.args[2])).then(amt => {
